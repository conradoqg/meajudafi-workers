...
const createTotalProgressInfo = () => {
    return (progress) => `CVMDataProcess Overall (${progress.total}): [${'▇'.repeat(progress.percentage) + '-'.repeat(100 - progress.percentage)}] ${progress.percentage.toFixed(2)}% - ${prettyMs(progress.elapsed)} - speed: ${progress.speed.toFixed(2)}r/s - eta: ${prettyMs(progress.eta)}`;
};
