    ...
    await Promise.all(dataList.map((data) => limit(async () => {
        ...
        if (shouldDownload) {
            ...
            try {
                ...
                try {
                    ui.start(`download-${data.url}`, 'Downloading', createStreamProgressInfo(`Downloading ${fundListURL} as ${localFilename}`), createDownloadStreamFinishInfo(`Downloaded ${localFilename}`));
                    ...
                    if (success) {
                        ...
                        ui.start(`read-${data.url}`, 'Transforming', createStreamProgressInfo(`Transforming ${fundListURL} as ${localFilename}`), createTransformStreamFinishInfo(`Transformed ${fundListURL} as ${localFilename}`));
