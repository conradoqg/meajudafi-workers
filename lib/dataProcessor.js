...
const isDownloadNecessary = async (filename, processedList) => {
    const result = await requestPromise.head(filename);

    return result && fileHasChanged(filename, result.etag, processedList);
};
            ...
            if (header == null) header = data;
                ...
                try {
                    ...
                    data.map((item, index) => {
                        ...
                        else preparedData[header[index]] = item ? item.replace(/'/g, '') : null;
                    });
        ...
        if (await isDownloadNecessary(fundListURL, processedList)) {
            ...
            const createUnknownProgressStream = (name) => {
                ...
                unknowProgress.on('progress', (progress) => {
                    console.log(`${name} file ${data.url}: #${progress.transferred}`);
                });
            ...
            try {
                ...
                if (!data.metadata.upsertConflictField) await db.pool.query(`DELETE FROM ${data.metadata.table}`);
