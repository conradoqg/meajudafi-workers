...
const createDailyCalculatorStream = () => {
    ...
    const processDaily = (stream, chunk, chunkBefore, firstChunkEver) => {
        ...
        const accSquaredDiffFromMeanSum = dailyReduce(returnsByYearAndMonth, (acc, value) => {
            return acc + Math.pow(value - accMeanReturn, 2);
        }, 0);
    ...
    const processYearly = (stream, chunk) => {
        ...
        if (returnsByYearAndMonth[referenceYearHash]) {
            ...
            // Sharpe
            const cdi_investiment_return = monthlyReduce(cdiReturnsByYearAndMonth[referenceYearHash], (acc, value) => acc * (1 + (value)), 1) - 1;
