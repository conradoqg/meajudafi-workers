    ...
    async extract(xpiID, fundName) {
        ...
        if (cnpj.length != 14) {
            ...
            if (foundCNPJs.length > 0) {
                ...
                console.log(`Fund ${fundName} (normalized as ${normalizedFundName}) found with rating ${bestMatch.rating}) as ${bestMatch.target}, ${bestMatch.rating >= 0.80 ? 'accepting' : 'ignoring'}`);
