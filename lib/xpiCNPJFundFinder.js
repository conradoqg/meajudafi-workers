    ...
    async extract(xpiID, fundName) {
        ...
        if (cnpj.length != 14) {
            ...
            const normalizedFundName = fundName
                .replace(/(^| )(FIRF)(|$)/ig, '$1FUNDO DE INVESTIMENTO RENDA FIXA$3')
                .replace(/(^| )(FIC)(|$)/ig, '$1FUNDO DE INVESTIMENTO EM COTAS$3')
            ...
            await page.type('#lst-ib', `${normalizedFundName} CNPJ`);
            await Promise.all([
            ...
            if (foundCNPJs.length > 0) {
                ...
                console.log(`stringSimilarity: ${fundName} found (${bestMatch.rating}) ${bestMatch.target}`);
