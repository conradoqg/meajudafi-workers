...
const createCalculatorStream = () => {
    ...
    const processDaily = (stream, chunk, chunkBefore, firstChunkEver) => {
        ...
        returns.push((chunk.vl_quota / chunkBefore.vl_quota) - 1);
        cdiReturns.push(chunkBefore.valor / 100);
        ...
        // Accumulated Risk
        let accEntries = returns.length;
        const accDailyReturnSum = returns.reduce((acc, value) => acc + value, 0);
        // const accDailyReturnSum = dailyReduce(returnsByYearAndMonth, (acc, value) => {
        //     accEntries++;
        //     return acc + value;
        // }, 0);
        const accMeanReturn = accDailyReturnSum / accEntries;
        const accSquaredDiffFromMeanSum = returns.reduce((acc, value) => acc + Math.pow(value - accMeanReturn, 2), 0);
        ...
        const cdi_accumulated_investiment_return = cdiReturns.reduce((acc, value) => acc * (1 + value), 1) - 1;
