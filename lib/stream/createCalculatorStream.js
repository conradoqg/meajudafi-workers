    ...
    return stream.Transform({
        ...
        transform: function (chunk, e, callback) {
            try {
                ...
                chunks.push(chunk);
                if (last21Chunks.length >= 21) {
                    const removedChunk = last21Chunks.shift();
                    let changeToRemove = returnFromQuota(removedChunk.vl_quota, last21Chunks[0].vl_quota);
                    last21Risk.removeMeasurement(changeToRemove);
                }
                last21Chunks.push(chunk);
                if (last63Chunks.length >= 63) {                    
                    const removedChunk = last63Chunks.shift();
                    let changeToRemove = returnFromQuota(removedChunk.vl_quota, last63Chunks[0].vl_quota);
                    last63Risk.removeMeasurement(changeToRemove);
                }
                last63Chunks.push(chunk);
                if (last126Chunks.length >= 126) {                    
                    const removedChunk = last126Chunks.shift();
                    let changeToRemove = returnFromQuota(removedChunk.vl_quota, last126Chunks[0].vl_quota);
                    last126Risk.removeMeasurement(changeToRemove);
                }
                last126Chunks.push(chunk);
                if (last252Chunks.length >= 252) {                    
                    const removedChunk = last252Chunks.shift();
                    let changeToRemove = returnFromQuota(removedChunk.vl_quota, last252Chunks[0].vl_quota);
                    last252Risk.removeMeasurement(changeToRemove);
                }
                last252Chunks.push(chunk);
                if (last504Chunks.length >= 504) {                    
                    const removedChunk = last504Chunks.shift();
                    let changeToRemove = returnFromQuota(removedChunk.vl_quota, last504Chunks[0].vl_quota);
                    last504Risk.removeMeasurement(changeToRemove);
                }
                last504Chunks.push(chunk);
                if (last756Chunks.length >= 756) {                    
                    const removedChunk = last756Chunks.shift();
                    let changeToRemove = returnFromQuota(removedChunk.vl_quota, last756Chunks[0].vl_quota);
                    last756Risk.removeMeasurement(changeToRemove);
                }
