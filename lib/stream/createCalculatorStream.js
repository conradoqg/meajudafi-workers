...
const createCalculatorStream = () => {
    ...
    const processDaily = (stream, chunk, chunkBefore) => {
        ...
        let ird_cdi_risk_1y = 0;
        let ird_cdi_risk_2y = 0;
        let ird_cdi_risk_3y = 0;
        let ird_cdi_accumulated_risk = 0;
        ...
        if (chunkBefore) {
            ...
            ird_cdi_risk_1y = last252Risk.get() * Math.sqrt(252);

            // Risk 2Y
            if (last504Risk == null) last504Risk = new StandardDeviation(change);
            else last504Risk.addMeasurement(change);
            ird_cdi_risk_2y = last504Risk.get() * Math.sqrt(252);

            // Risk 3Y
            if (last756Risk == null) last756Risk = new StandardDeviation(change);
            else last756Risk.addMeasurement(change);
            ird_cdi_risk_3y = last756Risk.get() * Math.sqrt(252);

            // Accumulated Risk
            if (risk == null) risk = new StandardDeviation(change);
            else risk.addMeasurement(change);
            ird_cdi_accumulated_risk = risk.get() * Math.sqrt(252);

            // Sharpe 1Y     
            ird_cdi_sharpe_1y = calcSharpeForPeriod(ird_cdi_risk_1y, ird_investment_return_1y, ird_cdi_investment_return_1y, last252Chunks.length - 1);

            // Sharpe 2Y                
            ird_cdi_sharpe_2y = calcSharpeForPeriod(ird_cdi_risk_2y, ird_investment_return_2y, ird_cdi_investment_return_2y, last504Chunks.length - 1);

            // Sharpe 3Y        
            ird_cdi_sharpe_3y = calcSharpeForPeriod(ird_cdi_risk_3y, ird_investment_return_3y, ird_cdi_investment_return_3y, last756Chunks.length - 1);

            // Accumulated Sharpe        
            ird_cdi_accumulated_sharpe = calcSharpeForPeriod(ird_cdi_accumulated_risk, ird_accumulated_investment_return, ird_cdi_accumulated_investment_return, chunks.length - 1);

            // Consistency 1Y        
            consistencyReachedLast252 = calcConsistencyForPeriod(ird_investment_return_1y, ird_cdi_investment_return_1y, 252, consistencyReachedLast252, last252Consistency);
            ird_cdi_consistency_1y = getConsistencyForPeriod(consistencyReachedLast252, last252Consistency);

            // Consistency 2Y
            consistencyReachedLast504 = calcConsistencyForPeriod(ird_investment_return_2y, ird_cdi_investment_return_2y, 504, consistencyReachedLast504, last504Consistency);
            ird_cdi_consistency_2y = getConsistencyForPeriod(consistencyReachedLast504, last504Consistency);

            // Consistency 3Y
            consistencyReachedLast756 = calcConsistencyForPeriod(ird_investment_return_3y, ird_cdi_investment_return_3y, 756, consistencyReachedLast756, last756Consistency);
            ird_cdi_consistency_3y = getConsistencyForPeriod(consistencyReachedLast756, last756Consistency);
    ...
    const processMonthly = (stream, chunk) => {
        ...
        if (risk) {
            ...
            // Months Risk            
            const irm_cdi_risk = risksByMonth[month].get() * Math.sqrt(252);

            // Risk 1Y            
            const irm_cdi_risk_1y = last252Risk.get() * Math.sqrt(252);

            // Risk 2Y            
            const irm_cdi_risk_2y = last504Risk.get() * Math.sqrt(252);

            // Risk 3Y            
            const irm_cdi_risk_3y = last756Risk.get() * Math.sqrt(252);

            // Accumulated Risk
            const irm_cdi_accumulated_risk = risk.get() * Math.sqrt(252);

            // Sharpe
            const irm_cdi_sharpe = calcSharpeForPeriod(irm_cdi_risk, irm_investment_return, irm_cdi_investment_return, chunksByDate[year][month].length);

            // Sharpe 1Y
            const irm_cdi_sharpe_1y = calcSharpeForPeriod(irm_cdi_risk_1y, irm_investment_return_1y, irm_cdi_investment_return_1y, last252Chunks.length - 1);

            // Sharpe 2Y
            const irm_cdi_sharpe_2y = calcSharpeForPeriod(irm_cdi_risk_2y, irm_investment_return_2y, irm_cdi_investment_return_2y, last504Chunks.length - 1);

            // Sharpe 3Y
            const irm_cdi_sharpe_3y = calcSharpeForPeriod(irm_cdi_risk_3y, irm_investment_return_3y, irm_cdi_investment_return_3y, last756Chunks.length - 1);

            // Accumulated Sharpe            
            const irm_cdi_accumulated_sharpe = calcSharpeForPeriod(irm_cdi_accumulated_risk, irm_accumulated_investment_return, irm_cdi_accumulated_investment_return, chunks.length - 1);

            // Consistency 1Y        
            const irm_cdi_consistency_1y = getConsistencyForPeriod(consistencyReachedLast252, last252Consistency);

            // Consistency 2Y
            const irm_cdi_consistency_2y = getConsistencyForPeriod(consistencyReachedLast504, last504Consistency);

            // Consistency 3Y
            const irm_cdi_consistency_3y = getConsistencyForPeriod(consistencyReachedLast756, last756Consistency);
    ...
    const processYearly = (stream, chunk) => {
        ...
        if (risk) {
            ...
            // Risk            
            const iry_cdi_risk = risksByYear[year].get() * Math.sqrt(252);

            // Risk 1Y            
            const iry_cdi_risk_1y = last252Risk.get() * Math.sqrt(252);

            // Risk 2Y            
            const iry_cdi_risk_2y = last504Risk.get() * Math.sqrt(252);

            // Risk 3Y            
            const iry_cdi_risk_3y = last756Risk.get() * Math.sqrt(252);

            // Accumulated Risk
            const iry_cdi_accumulated_risk = risk.get() * Math.sqrt(252);

            // Sharpe
            const iry_cdi_sharpe = calcSharpeForPeriod(iry_cdi_risk, iry_investment_return, iry_cdi_investment_return, Object.values(chunksByDate[year]).reduce((acc, month) => acc + month.length, 0));

            // Sharpe 1Y
            const iry_cdi_sharpe_1y = calcSharpeForPeriod(iry_cdi_risk_1y, iry_investment_return_1y, iry_cdi_investment_return_1y, last252Chunks.length - 1);

            // Sharpe 2Y
            const iry_cdi_sharpe_2y = calcSharpeForPeriod(iry_cdi_risk_2y, iry_investment_return_2y, iry_cdi_investment_return_2y, last504Chunks.length - 1);

            // Sharpe 3Y
            const iry_cdi_sharpe_3y = calcSharpeForPeriod(iry_cdi_risk_3y, iry_investment_return_3y, iry_cdi_investment_return_3y, last756Chunks.length - 1);

            // Accumulated Sharpe            
            const iry_cdi_accumulated_sharpe = calcSharpeForPeriod(iry_cdi_accumulated_risk, iry_accumulated_investment_return, iry_cdi_accumulated_investment_return, chunks.length - 1);

            // Consistency 1Y                    
            const iry_cdi_consistency_1y = getConsistencyForPeriod(consistencyReachedLast252, last252Consistency);

            // Consistency 2Y
            const iry_cdi_consistency_2y = getConsistencyForPeriod(consistencyReachedLast504, last504Consistency);

            // Consistency 3Y
            const iry_cdi_consistency_3y = getConsistencyForPeriod(consistencyReachedLast756, last756Consistency);
