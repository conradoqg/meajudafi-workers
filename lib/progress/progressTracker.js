class ProgressTracker {
    ...
    error() {
        if (this.state.status == 'running' || this.state.status == 'new') {
            this.recalculateTimmings();
            this.state.finish = new Date().getTime();
            this.state.status = 'errored';
        } else {
            throw new Error('Progress is not new or running to be errored');
        }
    }
        ...
        if (this.state.status == 'running' || this.state.status == 'new') {
            ...
            throw new Error('Progress is not new or running to be ended');
        }
    }
        ...
        if (this.state.status == 'running') {
            if (this.state.total > 0) {
                if (current <= this.state.total) {
                    ...
                    throw new Error(`There are more steps (${current}) than the expected (${this.state.total})`);
                }
            }
            this.recalculateTimmings();
        } else {
