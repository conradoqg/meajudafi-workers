...
const createTotalProgressInfo = () => {
    return (progress) => `BTGPactualFundWorker: Processing Data of ${progress.total} funds: [${'▇'.repeat(progress.percentage / 2) + '-'.repeat(100 / 2 - progress.percentage / 2)}] ${progress.percentage.toFixed(2)}% - ${prettyMs(progress.elapsed)} - speed: ${progress.speed.toFixed(2)}r/s - eta: ${Number.isFinite(progress.eta) ? prettyMs(progress.eta) : 'Unknown'}`;
};
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    ui.update('BTGPactualFundWorkerData', { elapsed: convertHrtime(process.hrtime(start)).milliseconds });
                    ui.stop('BTGPactualFundWorkerData');

                    ui.start('BTGPactualFundWorkerDataProcessing', 'BTGPactualFundWorker: Processing data', createTotalProgressInfo(), createTotalFinishInfo());                    
