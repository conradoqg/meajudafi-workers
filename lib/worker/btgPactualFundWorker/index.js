...
class BTGPactualFundWorker extends Worker {
    ...
    getData(mainClient, progress) {
        progress.start();
        return getFundsData(mainClient);
    }

    discovery(mainClient, progress, { funds, btgFunds }) {
        return discoverFunds(funds, btgFunds, progress);
    }

    updateDatabase(mainClient, progress, rowsToUpsert) {
        progress.start();
...
const discoverFunds = async (funds, btgFunds, progress) => {
