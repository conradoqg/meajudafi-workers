...
CONFIG.WTD_TOKEN = process.env.WTD_TOKEN;
...
const create_inf_diario_fi_urlList = () => {
    ...
    const yearsHist = Array((now.getFullYear() - 3 - 2005)).fill(null).reduce(result => {
    ...
    async work(options) {
        try {
            ...
            try {
                ...
                const createDataParser = (data) => [createCSVParserAndTransformerStream(data.metadata.formatMap, data.metadata.delimiter)];
