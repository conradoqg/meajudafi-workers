...
const createTotalProgressInfo = () => {
    return (progress) => `CVMDataWorker: Getting CVM data of ${progress.total} files: [${'▇'.repeat(progress.percentage / 2) + '-'.repeat(100 / 2 - progress.percentage / 2)}] ${progress.percentage.toFixed(2)}% - ${prettyMs(progress.elapsed)} - speed: ${progress.speed.toFixed(2)}r/s - eta: ${Number.isFinite(progress.eta) ? prettyMs(progress.eta) : 'Unknown'}`;
};

const createTotalFinishInfo = () => {
    return (progress) => `CVMDataWorker: Getting CVM data took ${prettyMs(progress.elapsed)} at ${progress.speed.toFixed(2)}r/s`;
};

const createStreamProgressInfo = (type) => {
    return (progress) => `CVMDataWorker: ${type}: [${'▇'.repeat(progress.percentage / 2) + '-'.repeat(100 / 2 - progress.percentage / 2)}] ${progress.percentage.toFixed(2)}% - ${prettyBytes(progress.speed)}/s - ${Number.isFinite(progress.eta) ? prettyMs(progress.eta * 1000) : 'Unknown'} remaining`;
};
const createStreamFinishInfo = (type) => {
    return (progress) => `CVMDataWorker: ${type} took ${prettyMs(progress.runtime * 1000)} at ${prettyBytes(progress.speed)}/s with total size of ${prettyBytes(progress.transferred)}`;
};
...
module.exports = async (db, dataList) => {
    ...
    ui.start('total', 'CVMDataWorker: Getting CVM data', createTotalProgressInfo(dataList.length), createTotalFinishInfo());
    ...
    for (const data of dataList) {
        ...
        if (shouldDownload) {
            ...
            try {
                ...
                try {
                    ui.start(`download-${data.url}`, 'CVMDataWorker: Downloading', createStreamProgressInfo(`Downloading ${data.name}`), createStreamFinishInfo(`Downloading ${data.name}`));
                    ...
                    if (success) {
                        ...
                        ui.start(`read-${data.url}`, 'CVMDataWorker: Transforming', createStreamProgressInfo(`Transforming ${data.name}`), createStreamFinishInfo(`Transforming ${data.name}`));
                        if (path.extname(data.url).toLowerCase() == '.zip') {
                            await promisePipe(
                                fs.createReadStream(localFilename),
                                readProgressStream,
                                unzipStream.Parse(),
                                createFileUnzipStream(createFileProcessorStream, db, data),
                                stream.Writable()
                            );
                        } else {
                            await promisePipe(
                                fs.createReadStream(localFilename),
                                readProgressStream,
                                ...createFileProcessorStream(db, data)
                            );
                        }
