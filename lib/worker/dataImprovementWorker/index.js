...
class DataImprovementWorker extends TransactionedWorker {
    ...
    async getData(mainClient, progress) {
        progress.start();
        return mainClient.query('SELECT DISTINCT(cnpj_fundo), denom_social FROM inf_cadastral_fi');
    }
    ...
    async improve(mainClient, progress, funds) {
        ...
        const createDataImprovementStream = () => stream.Transform({
            ...
            transform: async (chunk, e, callback) => {
                try {
                    ...
                    progress.log(`Error: DataImprovementStream: ${ex.stack}`);                    
                    callback(ex);
                }
            }
        });
        ...
        const dataImprovementPipe = promisePipe(
            ...
            createInsertPromiseStream(null, mainClient, progress),
