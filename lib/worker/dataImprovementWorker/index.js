...
class DataImprovementWorker extends TransactionedWorker {
    ...
    async getData(mainClient, progress) {
        progress.start();
        const query = 'SELECT DISTINCT ON (cnpj_fundo) cnpj_fundo, denom_social FROM inf_cadastral_fi ORDER BY cnpj_fundo ASC, dt_reg DESC';
    ...
    async improve(mainClient, progress, funds) {
        ...
        try {            
            return await dataImprovementPipe;
        } catch (ex) {
