...
const getFundsData = async (pool, progressCallback) => {
    ...
    try {
        ...
        xpiFunds = await Promise.all(xpiFundObjectList.map(async item => limiter(async () => {
            ...
            await page.goto(`https://institucional.xpi.com.br/investimentos/fundos-de-investimento/detalhes-de-fundos-de-investimento.aspx?F=${item.id}`, { waitUntil: ['load', 'networkidle2'], timeout: 0 });
