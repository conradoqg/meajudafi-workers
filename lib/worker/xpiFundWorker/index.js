...
class XPIFundWorker extends TransactionedWorker {
    ...
    async updateDatabase(mainClient, progress, rowsToUpsert) {
        ...
        try {
            ...
            throw new Error(`Query "${newQuery.padStart(CONFIG.QUERY_LOG_SIZE)}" failed`);
        }
    }
...
const getFundsData = async (client, progress) => {
    ...
    try {
        ...
        xpiFunds = await asyncPool(4, xpiFundObjectList, async item => {
            ...
            let success = false;
            let lastError = null;
            const URL = `https://institucional.xpi.com.br/investimentos/fundos-de-investimento/detalhes-de-fundos-de-investimento.aspx?F=${item.id}`;
            ...
            while (retriesLeft-- > 0) {
                try {
                    ...
                    progress.log(`Scrap of "${URL}" failed, ${retriesLeft} retries left, waiting 1000ms for next try...`);
                    await progress.delay(1000);
                    lastError = ex;
                }
            }
            if (!success) throw new Error(`Scraper get or parse of "${URL}" failed`, lastError);
            return item;
        });
    } finally {
        await browser.close();
