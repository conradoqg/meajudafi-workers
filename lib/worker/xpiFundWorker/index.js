...
const getFundsData = async () => {
    ...
    return allKeys({
        xpiFunds: got.get(URL, { headers: { 'ocp-apim-subscription-key': '602224c06c434f35b6352ca902c5020a' } }).then(result => JSON.parse(result.body)).catch(ex => { throw new Error(`HTTP get or parse of "${URL}" failed`, ex); })
    });
};
