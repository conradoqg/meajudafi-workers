...
const getFundsData = async () => {    
    const URL = 'https://api.xpi.com.br/investment-funds/yield-portal/v2/investment-funds?onlyrpps=false&family=null';
    return allKeys({
        xpiFunds: got.get(URL, {
            headers: {
                'accept': 'application/json, text/plain, */*',
                'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36',
                'ocp-apim-subscription-key': '602224c06c434f35b6352ca902c5020a',
                'origin': 'https://www.xpi.com.br',
                'referer': 'https://www.xpi.com.br/',
                'accept-language': 'pt-BR,pt;q=0.9,en;q=0.8,en-US;q=0.7'
            }
        }).then(result => JSON.parse(result.body)).catch(ex => { throw new Error(`HTTP get or parse of "${URL}" failed`, ex); })
    });
};
