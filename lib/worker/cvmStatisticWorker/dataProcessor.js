...
const createStreamFinishInfo = (type) => {
    return (progress) => `${type} took ${prettyMs(progress.runtime * 1000)} at ${prettyBytes(progress.speed)}/s with total size of ${prettyBytes(progress.transferred)}`;
};
    ...
    for (const data of dataList) {
        ...
        if (shouldDownload) {
            ...
            try {
                ...
                try {
                    ui.start(`download-${data.url}`, 'Downloading', createStreamProgressInfo(`Downloading ${data.name}`), createStreamFinishInfo(`Downloading ${data.name}`));
                    ...
                    if (success) {
                        ...
                        ui.start(`read-${data.url}`, 'Transforming', createStreamProgressInfo(`Transforming ${data.name}`), createStreamFinishInfo(`Transforming ${data.name}`));
