...
class CVMStatisticWorker extends PooledWorker {
    ...
    async getPendingFunds(pool, progress, options) {
        ...
        try {
            ...
            if (full)
                ...
                queryfundsCount = await mainClient.query('SELECT cnpj_fundo FROM inf_diario_fi JOIN funds_enhanced ON inf_diario_fi.cnpj_fundo = funds_enhanced.f_cnpj WHERE (xf_id IS NOT NULL OR bf_id IS NOT NULL OR mf_id IS NOT NULL) AND cnpj_fundo = \'22232927000190\' GROUP BY cnpj_fundo');
            } else
    ...
    async calculateStatistics(pool, progress, { queryfundsCount, fundsCount }) {
        ...
        const statisticsPipe = promisePipe(
            fundsStream,
            createFundCalculatorStream(pool, progress),
