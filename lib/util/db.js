...
class Db {
    ...
    async takeOffline() {
        return this.refreshMaterializedViews();
    }

    async refreshMaterializedViews() {
        const views = [
            'icf_with_xf_and_iry_of_last_year',
            'icf_with_xf_and_iry_and_f_of_last_year',
            'xf_with_irm_timeseries',
        ...
        const createTotalProgressInfo = () => {
            return (progress) => `Refreshing materialized view (${progress.view}) (${progress.total}): [${'▇'.repeat((progress.percentage / 2)) + '-'.repeat((100 - progress.percentage) / 2)}] ${progress.percentage.toFixed(2)}% - ${prettyMs(progress.elapsed)} - speed: ${progress.speed.toFixed(2)}v/s - eta: ${Number.isFinite(progress.eta) ? prettyMs(progress.eta) : 'Unknown'}`;
        };

        const createTotalFinishInfo = () => {
            return (progress) => `Refreshing materialized view took ${prettyMs(progress.elapsed)} at ${progress.speed.toFixed(2)}v/s`;
        };
        ...
        try {
            ...
            ui.start('total', 'Refreshing materialized view', createTotalProgressInfo(), createTotalFinishInfo());
