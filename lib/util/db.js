...
class Db {
    ...
    async takeOffline() {
        const client = await this.pool.connect();
        try {
            console.log('Refreshing xpi_funds_with_investment_return_monthly_complete_and_expanded_materialized');
            await client.query(refresh_xpi_funds_with_investment_return_monthly_complete_and_expanded_materialized);
            console.log('Refreshing inf_cadastral_fi_with_xpi_and_iryf_of_last_year');
