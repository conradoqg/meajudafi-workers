...
class Db {
    ...
    async takeOffline(refresh = true, progress) {
        refresh && await this.refreshMaterializedViews(progress);
    ...
    async updateWorkerInfo() {
        ...
        try {
            const versionUpsertQuery = Db.createUpsertQuery({
                table: 'about',
                primaryKey: ['what'],
                values: [{
                    'what': 'worker',
    ...
    async refreshMaterializedViews(progress) {
        const views = [
        ...
        if (progress) progress = new DefaultProgress(`${progress.progressTracker.id}.database.refreshMaterializedViews`);
        else progress = new DefaultProgress('Database');
    ...
    static createUpsertQuery(data) {
        ...
        const onConflictPart = (Array.isArray(data.primaryKey) ? data.primaryKey.map(key => '"' + key.toLowerCase() + '"').join(', ') : data.primaryKey);

        if (data.values.length > 0) {
            fieldsPart = Object.keys(data.values[0]).map(key => '"' + key.toLowerCase() + '"').join(', ');
            updateFieldsPart = Object.keys(data.values[0]).map(key => '"' + key.toLowerCase() + '"').slice(0).map(value => `${value} = excluded.${value}`).join(', ');
        }
