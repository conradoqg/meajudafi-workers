...
class UI {
    ...
    start(id, text, progressTemplate, finishTemplate, indertermined = false) {
        this.progress.start(text, indertermined);
    ...
    update(id, progress) {
        ...
        if (item) {
            ...
            const text = this.progressStates.filter(item => item.state).map(item => item.progressTemplate ? item.progressTemplate(item.state) : '').join('\n') + '\n';
