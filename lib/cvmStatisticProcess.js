...
const createTotalProgressInfo = () => {
    return (progress) => `Overall (${progress.total}): [${'▇'.repeat(progress.percentage) + '-'.repeat(100 - progress.percentage)}] ${progress.percentage.toFixed(2)}% - ${prettyMs(progress.elapsed)} - speed: ${progress.speed.toFixed(2)}r/s - eta: ${prettyMs(progress.eta)}`;
};

const createTotalFinishInfo = () => {
    return (progress) => `Overall took ${prettyMs(progress.elapsed)} at ${progress.speed.toFixed(2)}r/s`;
};
