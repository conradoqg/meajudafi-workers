    ...
    for (const data of dataList) {
        ...
        try {
            ...
            try {
                ...
                const foundProcessedURLResult = await client.query(`SELECT etag, last_modified from processed_url where url='${fundListURL}'`);
                if (foundProcessedURLResult.rowCount > 0) {
                    ...
                    } else if (etag && foundProcessedURLResult.rows[0].etag == etag) shouldDownload = false;
                }
            } catch (ex) {
